...
from .transforms import (GenLabel, RandomShift, ReplaceNoise, ScaleAmp,
                         StackRand, TransformSgram)


class WaveFormDataModule(pl.LightningDataModule):
    def __init__(self, data_conf: DataConfig, spec_conf: SpectrogramConfig, run_type: str):
        super().__init__()
        ...
        self.trans = {
            ...
            "sgram": TransformSgram(spec_conf)
        }
    ...
    def setup(self, stage: Optional[str] = None) -> None:
        sgram_transform = self.trans["sgram"] if self.data_conf.calculate_sgram else None
        if stage == "fit" or stage is None:
            ...
            self.wave_train = WaveFormDataset(
                self.data_conf, data_type="train", transform=transform_train, stack_transform=stack_transform_train, replace_noise_transform=replace_noise_transform_train, scale_at_end_transform=scale_at_end_transform_train, sgram_transform=sgram_transform)
            self.wave_val = WaveFormDataset(
                self.data_conf, data_type="val", transform=transform_val, sgram_transform=sgram_transform)
        ...
        if stage == "test" or stage is None:
            if self.run_type != "hyper_tune":
                ...
                self.wave_test = WaveFormDataset(
                    self.data_conf, data_type="test", transform=transform, sgram_transform=sgram_transform)
            else:
                transform = Compose([self.trans[key]
                                    for key in self.data_conf.val_trans if key not in ["stack", "replace_noise"]])
                self.wave_test = WaveFormDataset(
